            ...
            <div class="row">
                ...
                <div class="col-md-9 col-xl-9">
                    <main>
                        ...
                        <h2 class="u-h3 u-mb-small">Minify Javascript</h2>
								...
								<form class="c-search-form c-search-form--dark">
									...
									<div class="c-search-form__section">
										<div class="c-field has-icon-left">
											...
											<textarea class="c-input" id="txtJavascript"></textarea>
											<!--<small class="c-field__message">Use commas for multiple skills</small>-->
										</div>
									</div>

									<button class="c-btn c-btn--info" type="submit">Minify</button>
								</form>								
							</div>
						</div>
							...
							<div class="col-sm-12 col-lg-12 col-xl-12">
								<div class="c-search-form c-search-form--dark">
									...
									<div class="c-search-form__section">
										<div class="c-field has-icon-left">
											<!--<span class="c-field__icon"><i class="fa fa-user-o"></i></span>-->
											<label class="c-field__label c-search-form__label" for="search-position">Minified Javascript</label>
											<textarea class="c-input" id="txtMinifiedJS"></textarea>
		...
		<script>
			...
			$('button[type=submit]').click(function(e){
				...
				if(BlobBuilder) {
					...
					var aFileParts = [''+$('#txtJavascript').val()+''];
					oBuilder.append(aFileParts[0]);
					blob = oBuilder.getBlob("text\/plain"); // the blob
					
				} else {
					// everyone else
					blob = new Blob([''+$('#txtJavascript').val()+''], { 'type': 'text/plain' });    
				}
				
				var data = new FormData();
				data.append('jssource', blob);
				...
				$.ajax({
					...
					contentType: false,
					processData: false,
					url: "https://api.toolhub.cyou/WebsiteTools.php?action=minifyjs&query=infile",
					success: function(serverresponse){
						...
						if(response["success"] === 'true')
						{
							...
							$('#txtMinifiedJS').val(decodeURIComponent((response['result']['minified']).replace(/%(?![\da-f]{2})/gi,'%25')));
							$('.result').show();
						}
			...
			// $('button[type=submit]').click(function(e){
				...
				// // if($("#txtJavascript").val() != '')
				// // {
					// // var x = $("#txtJavascript").val().trim();
					// // //$('#txtMinifiedJS').val($("#txtJavascript").val().trim().replace(/\/\*([^*]|[\r\n]|(\*+([^*/]|[\r\n])))*\*+\//g,"").replace(/\s{1,}/g,' ').replace(/\{\s{1,}/g,"{").replace(/\}\s{1,}/g,"}").replace(/\;\s{1,}/g,";").replace(/\/\*\s{1,}/g,"/*").replace(/\*\/\s{1,}/g,"*/").trim());
					// // $('#txtMinifiedJS').val($("#txtJavascript").val().trim().replace('#(?s)\s|/\*.*?\*/|//[^\r\n]*#', '').trim());
					// // $('.result').show();
				// // }
				
				// //http://localhost:8080/seotoolstationclone/ContentTools.php?action=articlerewriter&query=
				// $.ajax({
					// //beforeSend: function() { textreplace(description); },
					// type: "GET",  
					// url: "http://localhost:8080/seotoolstationclone/WebsiteTools.php?action=minifyjs&query="+encodeURIComponent($('#txtJavascript').val()),
					...
					// success: function(serverresponse){
						...
						// {
							// //console.log(response['result']['minified']);
							// $('#txtMinifiedJS').val(decodeURIComponent(response['result']['minified']));
							// $('.result').show();
						// }
						// else if(response["success"] === 'false')
						// {
